...
export async function MobileSite({ params }: { params: Params }) {
  ...
  if (tab === "shops") {
    ...
    if (purchaseState === "complete") {
      content = completedOrder ? <div className="mx-auto flex max-w-lg flex-col gap-5 text-center"><div className="text-6xl">🎉</div><div><h1 className="font-display text-3xl font-black">購入が完了しました！</h1><p className="mt-2 text-muted-foreground">町のお店から、心をこめてお届けします。</p></div><Card className="text-left"><div className="flex justify-between gap-3 text-sm"><span className="text-muted-foreground">注文番号</span><span className="font-mono font-bold">{completedOrder.orderId.slice(0,8).toUpperCase()}</span></div><div className="mt-3 flex justify-between gap-3"><span>お支払い金額</span><span className="font-display font-black">{formatYen(completedOrder.totalAmount)}</span></div><div className="mt-4 rounded-xl bg-primary/10 p-4 text-center"><div className="font-bold text-primary">獲得ポイント</div><div className="font-display text-3xl font-black text-primary">+{completedOrder.pointsAwarded.toLocaleString()} pt</div><div className="mt-1 text-xs text-muted-foreground">現在の保有ポイント：{(account.profile?.points ?? 0).toLocaleString()} pt</div></div></Card><div className="grid grid-cols-2 gap-2"><a className={pill()} href="/?tab=account">ポイントを見る</a><a className={pill(true)} href="/?tab=home">ホームへ</a></div></div> : <Card><h1 className="font-display text-2xl font-black">購入は完了しています</h1><p className="mt-2 text-muted-foreground">注文情報の取得に時間がかかっています。アカウント画面でポイントをご確認ください。</p><a className={`${pill(true)} mt-4`} href="/?tab=account">アカウントへ</a></Card>
