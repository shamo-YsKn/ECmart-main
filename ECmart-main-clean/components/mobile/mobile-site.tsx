...
export async function MobileSite({ params }: { params: Params }) {
  ...
  if (tab === "shops") {
    ...
    content = <div className="flex flex-col gap-5"><div><h1 className="font-display text-3xl font-black">ロボット工房</h1><p className="text-muted-foreground">スマホ版は2D表示。各ボタンはページ再描画で確実に反映します。</p></div>
  ...
  return <div className="min-h-svh bg-background text-foreground"><header className="sticky top-0 z-40 border-b bg-background/95"><div className="mx-auto flex h-16 max-w-3xl items-center justify-between px-4"><a href="/?tab=home" className="font-display text-xl font-black">🔩 マチノワ</a><div className="text-sm">{account.user ? "ログイン中" : "ゲスト"}</div></div></header><main className="mx-auto max-w-3xl px-4 pb-28 pt-6">{one(params.favoriteError)&&<div className="mb-4 rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm">{one(params.favoriteError)}</div>}{content}</main><nav className="fixed inset-x-0 bottom-0 z-50 border-t bg-background"><div className="mx-auto grid max-w-3xl grid-cols-6">{TABS.map(([key,label])=><a key={key} href={q({tab:key})} className={`flex min-h-16 items-center justify-center px-1 text-center text-[0.68rem] font-bold ${tab===key?"text-primary":"text-muted-foreground"}`}>{label}{key==="cart"&&cartCount>0?` (${cartCount})`:""}</a>)}</div></nav></div>
}
