...
type MobilePageTab = typeof TABS[number][0] | "gacha" | "workbench"
...
export async function MobileSite({ params }: { params: Params }) {
  ...
  if (tab === "shops") {
    ...
    content = <div className="flex flex-col gap-5"><div><div className="text-sm font-bold text-primary">Phase 2-1</div><h1 className="font-display text-3xl font-black">2Dアイテム工作</h1></div><Card><p className="font-bold">このPhaseの自由配置エディタはPC版で利用できます。</p><p className="mt-2 text-sm text-muted-foreground">スマホ互換モードはReactのドラッグ操作に依存しない構成のため、今回は閲覧導線のみ維持しています。今後のPhaseでモバイル工作UIを追加できます。</p><a className={`${pill()} mt-4`} href="/?tab=robot">ロボット工房へ戻る</a></Card></div>
  } else if (tab === "cart") {
